...
func (m CreateOrderRequest) Validate() error {
	return validation.ValidateStruct(&m,
		...
		validation.Field(&m.ConsignessArea, validation.When(m.ConsignessArea.Valid, validation.Length(1, 55).Error("收件人地区不能超过 {{.max}} 个字符"))),
		...
		validation.Field(&m.ConsigneePhone,
			...
			validation.Length(1, 55).Error("收件人电话长度必须在 {{.min}} ~ {{.max}} 个字符"),
		),
